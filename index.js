...
/**
 * Convert envs to object then string {'port':3000,'nodeEnv':'prod'}
 */
   ...
   async (req, res) => {
      ...
      try {
         ...
         if (activeBuilds.has(repoId)) {
            console.log('Build already running for repoId:', repoId);
            return res.sendStatus(200);
         }
         ...
         console.log('All users with repos in DB:');
         allUsers.forEach((u) => {
            u.repos.forEach((r) => {
               console.log(
                  `user=${u.email} | repo=${r.name} | repoId=${r.repoId} | type=${typeof r.repoId}`
               );
            });
         });
         ...
         if (!user) {
            console.log(
               `Number match failed for repoId: ${repoId}, trying string match...`
            );
         ...
         if (!user) {
            console.log(
               `No user found for repoId: ${repoId} (tried both Number and String)`
            );
         ...
         const command = new RunTaskCommand({
            ...
            networkConfiguration: {
               awsvpcConfiguration: {
                  ...
                  subnets: [
                     'subnet-04f323939b63abec6',
                     'subnet-0a60e250f5830c75c',
                     'subnet-0b1dba884c5855587',
                  ],
                  securityGroups: ['sg-0392cb319058e58d1'],
               },
            },
            overrides: {
               containerOverrides: [
         ...
         console.log(
            `Auto deploy triggered for ${repo.name} → ${project.slug}`
         );
         return res.sendStatus(200);
      } catch (error) {
